...
4. The public ledger only shows *whether* each party confirmed and *whether* the settlement is finalized — never the underlying terms, amounts, or conditions
...
\`\`\`
├── divorce-settlement-contract/    # Compact smart contract + compiled ZK artifacts
│   ├── src/
│   │   ├── divorce.compact         # The Compact smart contract
│   │   ├── witnesses.ts            # Private state type definitions
│   │   ├── index.ts                # Contract exports
│   │   └── managed/divorce/        # Compiled contract (circuits, keys, zkir)
│   └── package.json
│
├── divorce-settlement-cli/         # CLI for deploying & interacting with the contract
│   ├── src/
│   │   ├── deploy.ts               # Deployment script
│   │   ├── api.ts                  # Contract interaction functions
│   │   ├── cli.ts                  # Interactive TUI for settlement actions
│   │   ├── common-types.ts         # TypeScript types for the contract
│   │   └── config.ts               # Network configurations
│   └── package.json
│
├── frontend-vite-react/            # React frontend (Vite)
