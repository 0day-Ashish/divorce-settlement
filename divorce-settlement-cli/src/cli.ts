...
import { type DivorceProviders, type DeployedDivorceContract } from './common-types';
...
║         ───────────────────────────                          ║
║         Privacy-preserving settlement compliance on-chain    ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
`;
...
${DIVIDER}
  Settlement Actions${dustBalance ? `                  DUST: ${dustBalance}` : ''}
${DIVIDER}
  [1] Deploy a new settlement contract
  [2] Join an existing settlement contract
  [3] Monitor DUST balance
  [4] Exit
${'─'.repeat(62)}
> `;

/** Build the settlement actions menu, showing current DUST balance in the header. */
const settlementMenu = (dustBalance: string) => `
${DIVIDER}
  Settlement Actions${dustBalance ? `                  DUST: ${dustBalance}` : ''}
${DIVIDER}
  [1] Confirm compliance (Party A)
  [2] Confirm compliance (Party B)
  [3] Finalize settlement
  [4] Display settlement status
  [5] Exit
${'─'.repeat(62)}
> `;
...
const joinContract = async (providers: DivorceProviders, rli: Interface): Promise<DeployedDivorceContract> => {
...
const deployOrJoin = async (
  ...
  rli: Interface,
): Promise<DeployedDivorceContract | null> => {
  while (true) {
    ...
    switch (choice.trim()) {
      case '1':
        try {
          const contract = await api.withStatus('Deploying divorce settlement contract', () =>
 ...
 */
const mainLoop = async (providers: DivorceProviders, walletCtx: api.WalletContext, rli: Interface): Promise<void> => {
  const divorceContract = await deployOrJoin(providers, walletCtx, rli);
  ...
  while (true) {
    const dustLabel = await getDustLabel(walletCtx.wallet);
    const choice = await rli.question(settlementMenu(dustLabel));
    switch (choice.trim()) {
      case '1':
        try {
          await api.withStatus('Party A confirming compliance', () => api.confirmByPartyA(divorceContract));
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Party A confirmation failed: ${msg}\n`);
        }
        break;
      case '2':
        try {
          await api.withStatus('Party B confirming compliance', () => api.confirmByPartyB(divorceContract));
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Party B confirmation failed: ${msg}\n`);
        }
        break;
      case '3':
        try {
          await api.withStatus('Finalizing settlement', () => api.finalizeSettlement(divorceContract));
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Finalization failed: ${msg}\n`);
        }
        break;
      case '4':
        await api.displaySettlementStatus(providers, divorceContract);
        break;
      case '5':
        return;
      default:
