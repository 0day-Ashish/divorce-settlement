...
import {
  DivorceCircuits,
  type DivorcePrivateStateId,
  type DivorceProviders,
  type DeployedDivorceContract,
} from './common-types';
...
// Pre-compile the divorce contract with ZK circuit assets
const divorceCompiledContract = CompiledContract.make('divorce', Divorce.Contract).pipe(
...
export const getDivorceLedgerState = async (
  providers: DivorceProviders,
  contractAddress: ContractAddress,
): Promise<{ isSettled: bigint; partyAConfirmed: bigint; partyBConfirmed: bigint } | null> => {
  ...
  const state = await providers.publicDataProvider.queryContractState(contractAddress).then((contractState) => {
    ...
    const l = Divorce.ledger(contractState.data);
    return { isSettled: l.isSettled, partyAConfirmed: l.partyAConfirmed, partyBConfirmed: l.partyBConfirmed };
  });
...
): Promise<DeployedDivorceContract> => {
  const divorceContract = await findDeployedContract(providers, {
    contractAddress,
    compiledContract: divorceCompiledContract,
    privateStateId: 'divorcePrivateState',
    initialPrivateState: {},
  });
  logger.info(`Joined contract at address: ${divorceContract.deployTxData.public.contractAddress}`);
  return divorceContract;
};

export const deploy = async (
  providers: DivorceProviders,
  privateState: DivorcePrivateState,
): Promise<DeployedDivorceContract> => {
  logger.info('Deploying divorce settlement contract...');
  const divorceContract = await deployContract(providers, {
    compiledContract: divorceCompiledContract,
    privateStateId: 'divorcePrivateState',
    initialPrivateState: privateState,
  });
  logger.info(`Deployed contract at address: ${divorceContract.deployTxData.public.contractAddress}`);
  return divorceContract;
};

export const confirmByPartyA = async (divorceContract: DeployedDivorceContract): Promise<FinalizedTxData> => {
  logger.info('Party A confirming compliance...');
  const finalizedTxData = await divorceContract.callTx.confirmByPartyA(1n);
  return finalizedTxData.public;
};

export const confirmByPartyB = async (divorceContract: DeployedDivorceContract): Promise<FinalizedTxData> => {
  logger.info('Party B confirming compliance...');
  const finalizedTxData = await divorceContract.callTx.confirmByPartyB(1n);
  return finalizedTxData.public;
};

export const finalizeSettlement = async (divorceContract: DeployedDivorceContract): Promise<FinalizedTxData> => {
  logger.info('Finalizing settlement...');
  const finalizedTxData = await divorceContract.callTx.finalizeSettlement();
  return finalizedTxData.public;
};

export const displaySettlementStatus = async (
  providers: DivorceProviders,
  divorceContract: DeployedDivorceContract,
): Promise<{
  state: { isSettled: bigint; partyAConfirmed: bigint; partyBConfirmed: bigint } | null;
  contractAddress: string;
}> => {
  const contractAddress = divorceContract.deployTxData.public.contractAddress;
  const state = await getDivorceLedgerState(providers, contractAddress);
  if (state === null) {
    logger.info(`There is no divorce contract deployed at ${contractAddress}.`);
  } else {
    logger.info(
      `Settlement Status - isSettled: ${state.isSettled}, partyA: ${state.partyAConfirmed}, partyB: ${state.partyBConfirmed}`,
    );
  }
...
export const configureProviders = async (walletContext: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<DivorceCircuits>(contractConfig.zkConfigPath);
  return {
    //AES-256-GCM + PBKDF2
    // WalletProvider for encryption uses Encryption Public Key (EPK)
    privateStateProvider: levelPrivateStateProvider<typeof DivorcePrivateStateId>({
